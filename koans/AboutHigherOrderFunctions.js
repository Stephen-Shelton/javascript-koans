...
describe("About Higher Order Functions", function () {
  ...
  it("should use 'forEach' for simple iteration", function () {
    ...
    expect(msg).toEqual('falsetruefalse');// <- string bc msg is a string
    expect(numbers).toEqual([1,2,3]);
  });
  ...
  it("should use range to generate an array", function() {
      expect(_.range(3)).toEqual([0,1,2]); //3 stands for 3 index values. Starts at 0
      expect(_.range(1, 4)).toEqual([1,2,3]); //start at 1, give 3 index values.
      expect(_.range(0, -4, -1)).toEqual([0,-1,-2,-3,]);
      //0 is 1st value, most negative is -4, -1 says to proceed by -1. 
  });

  it("should use flatten to make nested arrays easy to work with", function() {
      expect(_([ [1, 2], [3, 4] ]).flatten()).toEqual([1,2,3,4]);
  }); //flatten means to combine multiple nested arrays into 1 array.
